...
export class YaMap extends React.Component<YaMapProps> {
    ...
    public setCenter(center: {
        ...
        lat: number,
        zoom?: number
    }, zoom: number = center.zoom || 10, azimuth: number = 0, tilt: number = 0, duration: number = 0, animation: Animation = Animation.SMOOTH) {
        UIManager.dispatchViewManagerCommand(
            findNodeHandle(this),
    ...
    private processRoute(event: NativeSyntheticEvent<{
        id: string
    } & RoutesFoundEvent<DrivingInfo | MasstransitInfo>>) {
    ...
    private processRouteLength(event: NativeSyntheticEvent<{ id: string } & RouteLength>) {
